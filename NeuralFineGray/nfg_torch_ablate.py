...
class NeuralFineGrayTorch(nn.Module):
  ...
  def forward_multihead(self, x, horizon):
    ...
    sr = []
    t_input = horizon.unsqueeze(1)  # just scalar t_eval
    for outcome_competing in self.outcome:
      outcome = outcome_competing(torch.cat((x_rep, t_input), 1))  # no multiplication
      sr.append(-outcome)

    sr = torch.cat(sr, -1)
    return sr, log_beta, t_input
  
  def gradient(self, outcomes, horizon, e):
      return torch.ones_like(horizon)[:, 0]  # Dummy value to avoid breakage
